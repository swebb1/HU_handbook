...
Throws should go into space, rather than directly at the receiver. This allows the receiver to run onto the disc at speed and also reduces the margin of error on a throw. It is much easier for a cutter to adjust their feet than to change the path of a disc mid-flight!
...
- Try to **gain yards** where possible
  - Or lead the thrower into a continuation pass that gains yards
...
When a pass is made, try to keep the disc moving quickly and safely to maintain momentum. This might be a quick continuation throw to a well timed cut or some small-ball movement with players in close proximity. This keeps the offence flowing, creates space up-field and exploits gaps while the defence is still recovering.
  ...
  - **Take the first open option**
    - You might not get a better option
    - Keep the disc moving and the offence flowing
    - Don't rush into a pass. Open, means you are ready to throw!
...
throws <- list(
  throw(players,"B",from = "H1", to = "Reset",show=T,frame = 1:2,throw_frame = 3, label_pos = "end_up"),
...
The first pass in an offensive possession is often the hardest to make, especially if cutters are static and the defence is set.
  ...
  - If the defence are poaching and clogging space then try to **clear poaches** or move the disc around them
    - Cut early to flush the poaches out
    - Swing the disc to bigger spaces
