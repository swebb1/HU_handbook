...
base <- list(
  geom_hline(aes(yintercept = c(23,5)), colour="#DB127A", linetype="dashed"),
  geom_text(aes(x=33,y=21,label="Swing lane"), colour="#DB127A", vjust = 1.5, size = 3)
)
...
players <- list(
  ...
  y = c(45,60,60), show = F, alpha = 0.2, frame = 1:3),
  pitch_object(label = "B", x = c(5,12,12),
  y = c(55,55,51), show = F, alpha = 0.2, frame = 1:3),
  pitch_object(label = "C", x = c(35), frame = 1:3,
  y = c(42), show = F, alpha = 0.2,label_pos = "up"),
  pitch_object(label = "D", x = c(35), frame = 1:3,
  y = c(46), show = F, alpha = 0.2,label_pos = "up")
...
p3 <- plot_play(pitch = pitch_d,object_list = objects, arrow_list = arrows, pv = ppv2,default_obj_size = 4,show_all = T) +
  ...
  theme_void() +
  coord_fixed(ratio = 0.5) +
  labs(title = "3: Handler Give-and-Go", subtitle =  "Thrower dishes to a poached reset then immediately cuts") +
  theme(plot.title = element_text(size = 10), plot.subtitle = element_text(size = 8))
...
When the disc is on or near the sideline, the reset should position themselves towards the **centre** of the field, starting **slightly up-field** of the disc to open up the swing lane.
...
players <- list(
  ...
  pitch_object(label = "B", x = c(18),
  y = c(50,50,50,55), show = F, alpha = c(0.2), frame = 1:4),
...
They should be ready to fill into the reset space if the original reset clears out and offer good continuation cuts around the swing lane. Positioning and cutting options vary depending on the offence structure but the principles remain the same: 
...
base <- list(
  ...
  geom_text(aes(x=24,y=50,label="Throwing lane"), colour="#D15609", angle = 90, vjust = -0.5, size = 3)
)
...
Initiation is the first few passes of an offensive possession. It is important to get off to a good start and set up the offence for success.
...
The first pass in an offensive possession is often the hardest to make, especially if cutters are not in motion and the defence is set.

  - Look to hit **active spaces** first
  - If it isn't a stoppage, run a few **stall 0 cuts**
...
From a turnover we look to either play fast or slow, nothing in between.
  ...
  - **Fast**
    - Closest to the disc picks it up*
      - *Unless you are the deepest player and there is no one in support (in this case move upfield)
